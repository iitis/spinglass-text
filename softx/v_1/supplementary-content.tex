...
Table~\ref{tab:supp-crossover} reports matched CPU and GPU wall times for
identical solver configurations. Ratios are CPU wall time divided by GPU wall
time. A ratio below one indicates shorter CPU wall time. The CPU and GPU arms
ran in separate alternating processes on an Intel Xeon Platinum 8462Y+ and one
NVIDIA H100. All measurements used \texttt{Zipper},
\texttt{SquareSingleNode}, $\beta=3$, and \texttt{max\_states}=128. Search
breadth was held fixed to isolate device-dependent contraction work because the
branch-and-bound search runs on the host in both arms. The 36-spin and 128-spin
cells used seven repetitions per device. The 2048-spin cells used two because
of their higher cost. The summary reports the minimum wall time for each arm.
...
fields. Each state $\sigma$ therefore has an exactly degenerate global spin flip
$-\sigma$. To avoid counting this symmetry-related pair as two valleys,
...
At \texttt{max\_states}=1024, 49 of the 50 tested instance and temperature runs
remained within one valley. Their maximum quotient distance from a returned
state to the best returned state was at most 399. The sole exception was
instance 1 at $\beta=8$. Its returned energy was 8.12 above the lowest energy
among the five fixed-order $\beta$ runs for that instance. The run did not
return the exact global spin flip. We therefore interpret this point as a
degraded contraction rather than evidence of an additional low-energy valley.

Changing the contraction order produced a broader solution set. Taking one
state from each of the eight lattice transformations at $\beta=4$ produced two
or three distinct valleys on four of five instances. Pairwise quotient distances
reached 1249 of the possible 1250. The transformation sweep therefore increased
diversity while the relative energy spread across transformations remained
between $2.21\times10^{-4}$ and $5.74\times10^{-4}$.
...
distance percentiles. Its square markers report the median pairwise minimum,
median, and maximum across the five transformation-sweep instances.
...
7  & 8/13/19 & 8/12/19 & 32/37/42 & 6/10/13 & 323/328/332 \\
...
This protocol was adopted after a sequential protocol produced an invalid
performance result. In a recorded development measurement without a retained
raw log, timing the serial arm after a concurrent warm-up left the CUDA memory
pool in a different state. For identical work, one serial baseline increased
from 14.3~s to 38.8~s, a factor of approximately 2.7. The contaminated baseline
indicated a speed-up of $1.68\times$, while the corrected paired protocol gave
$0.92\times$. Reported sweep and ladder ratios use the interleaved protocol and
the reclamation steps described above.
...
One exploratory run that overlapped the full package test suite produced a
different energy for one of 30 points. It is excluded from the timing results
because the machine was not idle. The source of the difference remains
unresolved.
\FloatBarrier
...
The drivers use the \texttt{SpinGlassPEPS.jl} v2.0.0 API. The launcher runs the
local checkout selected by \texttt{PKG}. To reproduce the article values, check
out the v2.0.0 tag before launching the complete suite from the supplementary
...
\path{xtransform.csv} was reproduced byte for byte in a multithreaded run
recorded as \path{xtransform_multithread.csv}. The discarded-weight results in
\path{eps_stats.csv} record the tested CPU calculation.
...
Table~\ref{tab:supp-allocation} reads the committed isolated-change records in
\suppfile{alloc.csv}, whose post-change arms are not all regenerated by
\suppfile{run_all.sh}.
