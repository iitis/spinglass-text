...
empirical. Profiling on the tested RTX~5080 configuration shows the solver is
host-bound; moving the contraction
temporaries off the collected heap cuts a 2048-spin solve's allocated memory by
about two-thirds (90.9 to 32~GiB). On the tested Xeon/H100 system we also
...
validated only against an exact oracle, as the original benchmark had to do when
it certified ground states with CPLEX. Algorithms and an extensive benchmark
...
by each contractor, with explicit per-row eviction. The old design was thread-unsafe
by construction and had to be cleared by manual calls scattered through the search,
so this change is what makes concurrent solving possible at all.
...
reports $\sum_i\varepsilon_i$, which is the leading term of the accumulated
fidelity loss of a boundary-MPS sweep. Reported with it are $\max_i\varepsilon_i$,
the number of truncations in which the bond-dimension bound (rather than the
singular-value tolerance) forced a non-negligible discard, and the
retained-versus-offered dimensions. That last count answers a question users could
not previously ask. On a 128-spin instance, bond dimension $D=4$ gives
$\sum\varepsilon = 3.1\times10^{-4}$ with all 18 truncations forced by the bond
bound, retaining 108 of 307 singular values; at
$D=32$ the same solve gives $5.6\times10^{-14}$ with no truncation bond-limited,
retaining 192 of 592. A bond-limited count of zero means that no bond-capped
truncation discarded relative weight above the $10^{-12}$ reporting threshold,
so no change from raising $D$ is expected at the reported precision. Previously
that was a matter of guesswork.
...
distinct transformed contraction orders is an oracle-free consistency check; a spread
that is a sizeable fraction of the energy scale flags transformation-order-sensitive
disagreement, which may reflect contraction or search error.

\textbf{An inverse-temperature ladder.} $\beta$ is the most consequential
parameter and was also the least guidable: take it too small and the conditional
...
Where boundary-MPS construction dominates (2048-spin instance) a warm-started rung is
...
Concurrency pays on both tested devices (Table~\ref{tab:sweep}). On CPU the sweep scales
...
four-way. A single admitted solve ($c{=}1$) is a slight net loss, because the
driver's fixed overhead is not amortized when nothing overlaps; the gain begins at
...
distort any speed-up ratio measured against it. Any performance claim about this
solver therefore needs interleaved arms, with a full reclaim before every timed
section and paired per-round ratios.
...
the eight lattice transformations, $\beta=4$, median over five instances of the
pairwise minimum, median and maximum. Eight states from eight orders span the
configuration space; $1024$ states from one order do not.}
\label{fig:evr}
\end{figure}
...
The dense case at that size still favours the host by ${\approx}10\%$. Energies
agree in every cell. On the tested Xeon/H100 system, the CPU is preferable for exploratory
work in these smaller or dense configurations; the D-Wave-scale sparse regime at
large bond is where the device earns its place.
...
count rather than volume. The post-change allocation totals were reproduced on both
tested systems; the H100 run gives 32.3~GiB on CPU and 24.1~GiB on GPU.}
\label{fig:alloc}
\end{figure}

\textbf{Where the time actually goes.} Profiling on the tested RTX~5080
configuration explains both tables and redirects the obvious optimizations. On the
profiled 128-spin solve the device was not the bottleneck: host-side CUDA API calls
account for only about a quarter of wall time and the majority is host-side Julia
work. The profiled 128- and 2048-spin configurations were host-bound, which helps
explain why the CPU remains competitive across the measured crossover: accelerating
the tensor work removes none of the host-side bookkeeping.
...
letting a practitioner detect substantial contraction loss on a problem where no
exact answer is available, which is the regime the method exists for. It diagnoses
the contraction and does not by itself establish overall solver convergence or
solution quality. The consolidation and the deprecation shims make the
...
scripts behind the figures and tables, together with runnable drivers for every
result reported above, are provided with the manuscript sources in the
\texttt{benchmarks/} directory. The allocation figure reports allocated bytes; its
post-change values were reproduced on both tested systems, while its pre-change
values are recorded measurements of the superseded code.
...
and the sparse contraction path, which is where the device wins according to
