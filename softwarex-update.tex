...
a full solve rather than a microbenchmark. The abstract's ${\approx}1.15\times$ per-device figure summarizes these two allocation changes taken together; they contribute unequally by device, as the per-change numbers above show.
...
in that reproducibility that predates this update. The measurement drivers, raw per-run data, and figure scripts behind every figure and table here are provided with the manuscript sources in the \texttt{benchmarks/} directory, documented in its \texttt{README}.
